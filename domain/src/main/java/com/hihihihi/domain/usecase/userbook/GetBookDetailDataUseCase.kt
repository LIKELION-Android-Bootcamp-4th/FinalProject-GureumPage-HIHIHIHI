...
class GetBookDetailDataUseCase @Inject constructor(
    ...
    operator fun invoke(userBookId: String): Flow<BookDetailData> {
        ...
        val quotesFlow = quoteRepository.getQuotesByUserBookId(userBookId)

        return combine(userBookFlow, quotesFlow) { userBook, quotes ->
            BookDetailData(userBook, quotes, emptyList())
        }
    }
}
